...
# Get the git commit
if [ -f $GOPATH/src/github.com/openebs/device-localpv/GITCOMMIT ];
then
    GIT_COMMIT="$(cat $GOPATH/src/github.com/openebs/device-localpv/GITCOMMIT)"
else
...
echo -e "\nbuilding the Device Driver version :- $VERSION\n"
...
env GOOS=$GOOS GOARCH=$GOARCH go build -ldflags \
    "-X github.com/openebs/device-localpv/pkg/version.GitCommit=${GIT_COMMIT} \
    -X main.CtlName='${CTLNAME}' \
    -X github.com/openebs/device-localpv/pkg/version.Version=${VERSION} \
    -X github.com/openebs/device-localpv/pkg/version.VersionMeta=${VERSION_META}"\
    -o $output_name\
    ./cmd
